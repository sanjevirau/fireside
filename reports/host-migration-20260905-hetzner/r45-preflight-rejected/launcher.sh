#!/usr/bin/env bash
# Deployment evidence, not a new product candidate. No retry loop or intervention.
# Usage: bash deploy-b5fe1d5-then-r36.sh ATTEMPT_SLUG /absolute/green-ci.json
set -euo pipefail
...
phase5_candidate=b5fe1d51ea1fd0b5912cab0ae2d4f1d4d1f0987d
phase5_ci_run=33955150481
...
if [[ "$#" -ne 2 || ! "$1" =~ ^[a-zA-Z0-9][a-zA-Z0-9._-]{0,70}$ || "$2" != /* ]]; then
  printf 'Usage: %s NEW_ATTEMPT_SLUG /absolute/green-ci.json\n' "$0" >&2
  exit 64
fi
test "$(id -un)" = sanjevi
...
for phase5_stack in stack-official stack-fireside fresh-colleague; do
  test "$(git -C "$phase5_root/$phase5_stack" rev-parse HEAD)" = "$phase5_twodart"
  test -z "$(git -C "$phase5_root/$phase5_stack" status --porcelain --untracked-files=no)"
done

printf 'Pre-build replacement-host hardware, inputs, and quiescent preflight.\n'
node "$phase5_attempt/hetzner-preflight.mjs" "$phase5_attempt/preflight-before-build"
...
phase5_verify_candidate
node "$phase5_attempt/hetzner-preflight.mjs" "$phase5_attempt/preflight-before-smoke"
phase5_common=(
...
node "$phase5_attempt/hetzner-preflight.mjs" "$phase5_attempt/preflight-before-full"
