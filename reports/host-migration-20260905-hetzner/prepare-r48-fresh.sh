...
phase5_preparation="$phase5_root/preparation-r48"
phase5_fresh="$phase5_root/fresh-acceptance/r48/fresh-colleague"
test "$(id -un)" = sanjevi
test ! -e "$phase5_fresh"
...
node "$phase5_root/deployment-3407c65/hetzner-preflight.mjs" "$phase5_root/attempts/r48-preparation-preflight" 3407c658d31fbedc35fced8670a6afffd2943e97
...
for(const stack of ['official','fireside']) {
  ...
  await writeFile(root+'/preparation-r48/'+stack+'-r47-owned-ports.patch',patch,{flag:'wx'});
...
await writeFile(root+'/preparation-r48/owned-port-restoration.json',JSON.stringify({at:new Date().toISOString(),receipt},null,2)+'\n',{flag:'wx'});
NODE
mkdir -p "$phase5_root/fresh-acceptance/r48"
...
const root='/srv/dev-fast/runtime-data/fireside-templates-hetzner-20260905';
const fresh=root+'/fresh-acceptance/r48/fresh-colleague';
...
await writeFile(root+'/preparation-r48/fresh-ports.json',JSON.stringify(phase5PortEnvironment(ports),null,2)+'\n',{flag:'wx'});
NODE
cd "$phase5_fresh"
test -d .git
test -d node_modules
