...
phase5_smoke="$phase5_root/diagnostics/two-tier-smoke-v3-20260903-b1ef52b-r28"
phase5_full="$phase5_root/full-gates/full-gate-v3-20260903-b1ef52b"
export CARGO_TARGET_DIR="$phase5_root/target-b1ef52b"
cd "$phase5_root/harness-b1ef52b"
test "$(git rev-parse HEAD)" = b1ef52b77846db7c94ab1c37451d977a629d69ac
...
test "$(<"$phase5_root/build-b1ef52b.exit")" = 0
sha256sum -c "$phase5_root/build-b1ef52b.SHA256SUMS"
node -e 'const fs=require("node:fs"); const c=JSON.parse(fs.readFileSync(process.argv[1])); if(c.headSha!=="b1ef52b77846db7c94ab1c37451d977a629d69ac" || c.databaseId!==33760383375 || c.conclusion!=="success" || c.jobs.length!==7 || c.jobs.some(j=>j.conclusion!=="success")) throw Error("all seven exact-candidate CI jobs must pass");' "$phase5_root/ci-b1ef52b-seven-jobs.json"
test ! -e "$phase5_smoke"
...
printf 'Starting r28 cheap smoke: official then Fireside\n'
set +e
...
cp "$phase5_root/ci-b1ef52b-seven-jobs.json" "$phase5_full/ci.json"
cp "$phase5_root/build-b1ef52b.SHA256SUMS" "$phase5_full/release-binary.SHA256SUMS"
