...
phase5_candidate=b1ef52b77846db7c94ab1c37451d977a629d69ac
phase5_checkout="$phase5_root/harness-b1ef52b"
export CARGO_TARGET_DIR="$phase5_root/target-b1ef52b"
test ! -e "$phase5_root/build-b1ef52b.exit"
test ! -e "$phase5_root/build-b1ef52b.SHA256SUMS"
test ! -e "$phase5_checkout"
test ! -e "$CARGO_TARGET_DIR"
trap 'phase5_build_status=$?; printf "%s\n" "$phase5_build_status" > "$phase5_root/build-b1ef52b.exit"; date --iso-8601=seconds' EXIT
date --iso-8601=seconds
node -e 'const fs=require("node:fs"); const c=JSON.parse(fs.readFileSync(process.argv[1])); if(c.headSha!=="b1ef52b77846db7c94ab1c37451d977a629d69ac" || c.databaseId!==33760383375 || c.conclusion!=="success" || c.jobs.length!==7 || c.jobs.some(j=>j.conclusion!=="success")) throw Error("all seven exact-candidate CI jobs must pass before build");' "$phase5_root/ci-b1ef52b-seven-jobs.json"
...
sha256sum "$CARGO_TARGET_DIR/release/fireside" > "$phase5_root/build-b1ef52b.SHA256SUMS"
cat "$phase5_root/build-b1ef52b.SHA256SUMS"
test -z "$(git status --porcelain --untracked-files=no)"
