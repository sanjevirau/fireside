...
phase5_candidate=3a5644f15f8a53912da1bf004fb354246fa5a92e
phase5_short=3a5644f
phase5_ci_run=33863537340
...
phase5_smoke="$phase5_root/diagnostics/two-tier-smoke-v3-20260904-30e0d25-r36"
phase5_harness="$phase5_root/harness-$phase5_short-r36-continuation"
phase5_target="$phase5_root/target-$phase5_short-r36-continuation"
phase5_full="$phase5_root/full-gates/full-gate-v3-20260904-$phase5_short-r36-fireside-continuation"
phase5_ci="$phase5_root/ci-$phase5_short-r36-continuation-seven-jobs.json"
phase5_build_log="$phase5_root/build-$phase5_short-r36-continuation.log"
phase5_build_exit="$phase5_root/build-$phase5_short-r36-continuation.exit"
phase5_build_sums="$phase5_root/build-$phase5_short-r36-continuation.SHA256SUMS"
phase5_launcher="$phase5_root/fireside-phase5-$phase5_short-r36-continuation.sh"
phase5_controller_log="$phase5_root/controller-$phase5_short-r36-continuation.log"
phase5_controller_exit="$phase5_root/controller-$phase5_short-r36-continuation.exit"
...
if [ "$phase5_status" -ne 0 ]; then
  printf 'Fresh checkout or release build failed; no Fireside measurement started.\n'
  exit "$phase5_status"
fi
...
cp -a -l "$phase5_original/inputs/full-data" "$phase5_full/inputs/full-data"
...
date --iso-8601=seconds
printf 'Starting immutable r36 Fireside continuation; official stage is preserved and not rerun.\n'
set +e
npm run test:suite:phase5-gate --prefix conformance -- "${phase5_common[@]}" \
  --full-data "$phase5_full/inputs/full-data" \
  --smoke-evidence "$phase5_smoke" \
  --official-baseline-evidence "$phase5_baseline" \
  --output-dir "$phase5_full/evidence" \
  --report-path "$phase5_full/report.md" \
