...
phase5_smoke="$phase5_root/diagnostics/two-tier-smoke-v3-20260903-4d6cf2f-r27"
phase5_full="$phase5_root/full-gates/full-gate-v3-20260903-4d6cf2f"
export CARGO_TARGET_DIR="$phase5_root/target-4d6cf2f"
cd "$phase5_root/harness-4d6cf2f"
test "$(git rev-parse HEAD)" = 4d6cf2ff90cdb33ec076c23067807303e304e255
...
test "$(<"$phase5_root/build-4d6cf2f.exit")" = 0
sha256sum -c "$phase5_root/build-4d6cf2f.SHA256SUMS"
node -e 'const fs=require("node:fs"); const c=JSON.parse(fs.readFileSync(process.argv[1])); if(c.headSha!=="4d6cf2ff90cdb33ec076c23067807303e304e255" || c.databaseId!==33755740999 || c.conclusion!=="success" || c.jobs.length!==7 || c.jobs.some(j=>j.conclusion!=="success")) throw Error("all seven exact-candidate CI jobs must pass");' "$phase5_root/ci-4d6cf2f-seven-jobs.json"
test ! -e "$phase5_smoke"
...
printf 'Starting r27 cheap smoke: official then Fireside\n'
set +e
...
cp "$phase5_root/ci-4d6cf2f-seven-jobs.json" "$phase5_full/ci.json"
cp "$phase5_root/build-4d6cf2f.SHA256SUMS" "$phase5_full/release-binary.SHA256SUMS"
