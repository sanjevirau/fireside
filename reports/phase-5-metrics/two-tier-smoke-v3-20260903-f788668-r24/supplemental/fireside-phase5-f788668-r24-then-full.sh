...
phase5_smoke="$phase5_root/diagnostics/two-tier-smoke-v3-20260903-f788668-r24"
phase5_full="$phase5_root/full-gates/full-gate-v3-20260903-f788668"
export CARGO_TARGET_DIR="$phase5_root/target-f788668"
cd "$phase5_root/harness-f788668"
test "$(git rev-parse HEAD)" = f788668a13c1f78817799bc7944da4c4592d5f67
...
test "$(<"$phase5_root/build-f788668.exit")" = 0
sha256sum -c "$phase5_root/build-f788668.SHA256SUMS"
node -e 'const fs=require("node:fs"); const c=JSON.parse(fs.readFileSync(process.argv[1])); if(c.headSha!=="f788668a13c1f78817799bc7944da4c4592d5f67" || c.databaseId!==33740062835 || c.conclusion!=="success" || c.jobs.length!==7 || c.jobs.some(j=>j.conclusion!=="success")) throw Error("all seven exact-candidate CI jobs must pass");' "$phase5_root/ci-f788668-seven-jobs.json"
test ! -e "$phase5_smoke"
...
printf 'Starting r24 cheap smoke: official then Fireside\n'
set +e
...
cp "$phase5_root/ci-f788668-seven-jobs.json" "$phase5_full/ci.json"
cp "$phase5_root/build-f788668.SHA256SUMS" "$phase5_full/release-binary.SHA256SUMS"
