...
phase5_smoke="$phase5_root/diagnostics/two-tier-smoke-v3-20260903-a6c66b4-r26"
phase5_full="$phase5_root/full-gates/full-gate-v3-20260903-a6c66b4"
export CARGO_TARGET_DIR="$phase5_root/target-a6c66b4"
cd "$phase5_root/harness-a6c66b4"
test "$(git rev-parse HEAD)" = a6c66b493480a6b716c9f32312cd52733a61dad0
...
test "$(<"$phase5_root/build-a6c66b4.exit")" = 0
sha256sum -c "$phase5_root/build-a6c66b4.SHA256SUMS"
node -e 'const fs=require("node:fs"); const c=JSON.parse(fs.readFileSync(process.argv[1])); if(c.headSha!=="a6c66b493480a6b716c9f32312cd52733a61dad0" || c.databaseId!==33752732445 || c.conclusion!=="success" || c.jobs.length!==7 || c.jobs.some(j=>j.conclusion!=="success")) throw Error("all seven exact-candidate CI jobs must pass");' "$phase5_root/ci-a6c66b4-seven-jobs.json"
test ! -e "$phase5_smoke"
...
printf 'Starting r26 cheap smoke: official then Fireside\n'
set +e
...
cp "$phase5_root/ci-a6c66b4-seven-jobs.json" "$phase5_full/ci.json"
cp "$phase5_root/build-a6c66b4.SHA256SUMS" "$phase5_full/release-binary.SHA256SUMS"
