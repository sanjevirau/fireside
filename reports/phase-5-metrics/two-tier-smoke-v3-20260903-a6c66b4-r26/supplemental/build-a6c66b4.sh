...
phase5_candidate=a6c66b493480a6b716c9f32312cd52733a61dad0
phase5_checkout="$phase5_root/harness-a6c66b4"
export CARGO_TARGET_DIR="$phase5_root/target-a6c66b4"
test ! -e "$phase5_root/build-a6c66b4.exit"
test ! -e "$phase5_root/build-a6c66b4.SHA256SUMS"
test ! -e "$phase5_checkout"
test ! -e "$CARGO_TARGET_DIR"
trap 'phase5_build_status=$?; printf "%s\n" "$phase5_build_status" > "$phase5_root/build-a6c66b4.exit"; date --iso-8601=seconds' EXIT
date --iso-8601=seconds
node -e 'const fs=require("node:fs"); const c=JSON.parse(fs.readFileSync(process.argv[1])); if(c.headSha!=="a6c66b493480a6b716c9f32312cd52733a61dad0" || c.databaseId!==33752732445 || c.conclusion!=="success" || c.jobs.length!==7 || c.jobs.some(j=>j.conclusion!=="success")) throw Error("all seven exact-candidate CI jobs must pass before build");' "$phase5_root/ci-a6c66b4-seven-jobs.json"
...
sha256sum "$CARGO_TARGET_DIR/release/fireside" > "$phase5_root/build-a6c66b4.SHA256SUMS"
cat "$phase5_root/build-a6c66b4.SHA256SUMS"
test -z "$(git status --porcelain --untracked-files=no)"
