  ...
  try {
    ...
    for (const name of names) {
      const response = await fetch(
        `${origin}/upload/storage/v1/b/synthetic-objects.example.test/o?uploadType=media&name=${encodeURIComponent(name)}`,
        {
          method: "POST",
...
async function listFireside(
  ...
  const response = await fetch(
    `${origin}/${route}/b/synthetic-objects.example.test/o?${query.toString()}`,
