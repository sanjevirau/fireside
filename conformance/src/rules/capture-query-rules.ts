...
import { verifyBrowserCapture, type BrowserCapture } from "./query-rules-verification.ts";
...
try {
  ...
  const [ownedPath, ownedFields] = queryRuleSeeds.find(([path]) => path === "records/owned")!;
  ...
  await writeFile(diagnosticLog, logs.join(""), { flag: "wx" });
  console.log(JSON.stringify({ diagnosticLog, output }));
