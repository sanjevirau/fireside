...
export const queryRuleCases: QueryRuleCase[] = [
  { id: "owner-equality", collection: "records", filter: owner, orderBy: [["updatedAt", "desc"]], limit: 12 },
  { id: "owner-absent", collection: "records" },
  { id: "owner-wrong-uid", collection: "records", filter: eq("createdBy", "other-owner") },
  { id: "owner-unverified", collection: "records", filter: owner, unverified: true },
  { id: "owner-in-single", collection: "records", filter: field("createdBy", "in", [queryRulesUid]) },
  { id: "owner-in-mixed", collection: "records", filter: field("createdBy", "in", [queryRulesUid, "other-owner"]) },
  { id: "owner-array-is-not-equality", collection: "records", filter: field("createdBy", "array-contains", queryRulesUid) },
  { id: "owner-inequality", collection: "records", filter: field("createdBy", ">=", queryRulesUid) },
  { id: "owner-bounded-equality", collection: "records", filter: and(field("createdBy", ">=", queryRulesUid), field("createdBy", "<=", queryRulesUid)) },
  { id: "owner-compound-and", collection: "records", filter: and(owner, eq("published", false)) },
  { id: "owner-compound-or-safe", collection: "records", filter: or(and(owner, eq("published", false)), and(owner, eq("published", true))) },
  { id: "owner-compound-or-unsafe", collection: "records", filter: or(owner, eq("published", true)) },
  { id: "owner-empty", collection: "emptyRecords", filter: owner },
  { id: "owner-empty-unconstrained", collection: "emptyRecords" },
  { id: "owner-empty-wrong-uid", collection: "emptyRecords", filter: eq("createdBy", "other-owner") },
  { id: "owner-nonmatching-extra-filter", collection: "records", filter: and(owner, eq("updatedAt", -1)) },
  { id: "permission-get-allowed", collection: "records", filter: eq("permissionId", "granted") },
  { id: "permission-get-denied", collection: "records", filter: eq("permissionId", "denied") },
  { id: "permission-get-missing", collection: "records", filter: eq("permissionId", "missing") },
  { id: "permission-in-all-granted", collection: "records", filter: field("permissionId", "in", ["granted", "granted2"]) },
  { id: "permission-in-mixed", collection: "records", filter: field("permissionId", "in", ["granted", "denied"]) },
  ...
  { id: "exists-constrained-path", collection: "dynamicExists", filter: eq("permissionId", "granted") },
  { id: "exists-missing-path", collection: "dynamicExists", filter: eq("permissionId", "missing") },
  ...
  { id: "group-root-only-rule", collection: "records", group: true, filter: owner },
...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    function permissionAccess(id) { return get(/databases/$(database)/documents/permissions/$(id)).data.uid == request.auth.uid; }
    function recordRead() { return verified() && (owner() || (resource.data.permissionId != null && permissionAccess(resource.data.permissionId))); }
    match /records/{id} { allow read: if recordRead(); }
    match /emptyRecords/{id} { allow read: if recordRead(); }
    match /shortCircuit/{id} { allow list: if owner() || get(/databases/$(database)/documents/permissions/missing).data.uid == request.auth.uid; }
    ...
    match /fixedGet/{id} { allow list: if get(/databases/$(database)/documents/permissions/granted).data.uid == request.auth.uid; }
    match /fixedExists/{id} { allow list: if exists(/databases/$(database)/documents/permissions/granted); }
    match /dynamicExists/{id} { allow list: if exists(/databases/$(database)/documents/permissions/$(resource.data.permissionId)); }
...
export const queryRuleSeeds: [string, Record<string, unknown>][] = [
  ["permissions/granted", { uid: queryRulesUid }],
  ["permissions/granted2", { uid: queryRulesUid }],
  ["permissions/denied", { uid: "other-owner" }],
  ...["records", "shortCircuit", "shared", "ratings", "nonnull", "notblocked", "limited", "offsets", "ordering", "fixedGet", "fixedExists", "dynamicExists", "groupDocs", "parents/p1/groupDocs"].flatMap((collection): [string, Record<string, unknown>][] => [
    [`${collection}/owned`, { createdBy: queryRulesUid, updatedAt: 2, permissionId: "denied", published: false, members: [queryRulesUid], score: 7, value: "safe" }],
    [`${collection}/other`, { createdBy: "other-owner", updatedAt: 1, permissionId: "granted", published: true, members: ["other-owner"], score: 4, value: "blocked" }],
  ]),
];
