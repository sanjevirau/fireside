...
const DEFAULT_BUCKET = "demo-synthetic-app.appspot.com";
const ASSETS_BUCKET = "synthetic-objects.example.test";
...
const rulesDirectory = fileURLToPath(new URL("../../../crates/storage-front/testdata/", import.meta.url));
const defaultRulesPath = join(rulesDirectory, "synthetic-storage.default.rules");
const assetsRulesPath = join(rulesDirectory, "synthetic-storage.assets.rules");
const defaultRules = await readFile(defaultRulesPath, "utf8");
const assetsRules = await readFile(assetsRulesPath, "utf8");
const DEFAULT_RULES_SHA256 = sha256(defaultRules);
const ASSETS_RULES_SHA256 = sha256(assetsRules);
...
try {
    ...
    rules: [
      {
        ...
        rules: { name: "synthetic-storage.default.rules", content: defaultRules },
      },
      {
        resource: ASSETS_BUCKET,
        rules: { name: "synthetic-storage.assets.rules", content: assetsRules },
      },
    ],
  });
  ...
  await writeFixture("storage-multi-bucket-rules-and-import-export", {
    ...
    capturedAt: new Date().toISOString(),
    hypothesis:
      "Two independent synthetic bucket policies remain distinct, and the official buckets/blobs/metadata export layout re-imports byte-identically without lifecycle events",
    credentialsStored: false,
    accessTokensStored: false,
    realUserDataStored: false,
    sourceHashes,
...
async function writeFixture(name: string, fixture: unknown): Promise<void> {
  ...
  await writeFile(join(root, "fixture.json"), fixtureText, { encoding: "utf8", flag: "wx" });
  await writeFile(join(root, "decoded-contract.json"), decodedText, { encoding: "utf8", flag: "wx" });
  await writeFile(
