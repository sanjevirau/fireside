...
The 22 exchanges include owner-seeded documents, root/nested collection IDs with
an empty POST body, owner-only metadata discovery, document listing/order/masks,
missing-parent placeholders, denial and full-page terminal tokens followed by an
empty third page on both listing APIs. Server
...
budget, in three isolated attempts. `status: null` records the last timeout; it is
