...
impl FirestoreService {
    ...
    /// Queue before entering the blocking pool: a burst of durable writes must
    /// not occupy every async worker (or one blocking thread per queued write).
    /// The closure retains the existing rules/transaction critical section.
    /// Once started, it also retains the permit if the caller disconnects, so
    /// no other queued task overlaps an in-flight durable commit.
    async fn run_write<T, F>(&self, operation: F) -> Result<T, Status>
    where
        T: Send + 'static,
        F: FnOnce(Self) -> Result<T, Status> + Send + 'static,
    {
        let service = self.clone();
        let Some(queue) = &self.disk_write_queue else {
            return operation(service);
        };
        let permit = Arc::clone(queue)
            .acquire_owned()
            .await
            .map_err(|_| Status::unavailable("disk write queue closed"))?;
        tokio::task::spawn_blocking(move || {
            let _permit = permit;
            operation(service)
        })
        .await
        .map_err(|_| Status::internal("disk write task failed"))?
    }
    ...
    pub(crate) async fn apply_stream_writes(
        &self,
        authorization: &Authorization,
        database: &DatabaseName,
        ...
        self.run_write(move |service| {
            let _guard = service.write_lock();
            let snapshot = service.store.snapshot();
            service.authorize_writes(&authorization, &database, &decoded, &snapshot, now())?;
            service.apply_writes(&decoded)
        })
        .await
    }
...
impl Firestore for FirestoreService {
    ...
    async fn commit(
        ...
        self.run_write(move |service| {
            let _guard = service.write_lock();
            let current = service.store.snapshot();
            ...
            service.authorize_writes(&authorization, &database, &decoded, &current, now())?;
            Ok(Response::new(service.apply_writes(&decoded)?))
        })
        .await
    }
    ...
    async fn batch_write(
        ...
        self.run_write(move |service| {
            ...
            for write in decoded {
                let snapshot = service.store.snapshot();
                let authorized = service.authorize_writes(
                    &authorization,
                    &database,
                ...
                match authorized.and_then(|()| service.apply_writes(&[write])) {
                    Ok(response) => {
...
mod tests {
    ...
    async fn security_rules_enforce_stream_writes_and_live_listen_rechecks() {
        ...
        service
            ...
            .expect("deny reload");
        // Even a commit outside the target scope must recheck authorization;
        // skipping result reconstruction is not permission to skip rules.
        service
            .store()
            .commit(&[Write::Set {
                key: DocumentKey::new(
