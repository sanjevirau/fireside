...
impl ExportedDocument {
    ...
    /// Takes ownership of the decoded key and fields without cloning a record.
    #[must_use]
...
mod tests {
    ...
    #[test]
    fn consuming_official_entities_preserves_fields_without_cloning() {
        for record in oracle_records() {
            let document = decode_entity(&record).expect("official entity should decode");
            let first_field = document
                .fields()
                .first_key_value()
                .map(|(_, value)| std::ptr::from_ref(value));
            let expected = encode_entity(&document).unwrap();
            let (key, fields) = document.into_parts();
            assert_eq!(
                fields
                    .first_key_value()
                    .map(|(_, value)| std::ptr::from_ref(value)),
            ...
            assert_eq!(
                encode_entity(&ExportedDocument::new(key, fields)).unwrap(),
                expected
            );
        }
    }
