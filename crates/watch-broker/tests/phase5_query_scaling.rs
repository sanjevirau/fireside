...
const RECORD_DOCUMENTS: usize = 100;
...
const QUERY_COLLECTIONS: [&str; 11] = [
    "collection00",
    "collection01",
    "collection02",
    "collection03",
...
fn measure_parallel_collections(
            ...
            QUERY_COLLECTIONS
                ...
                .map(|collection| {
                    ...
                    scope.spawn(move || {
                        ...
                        if *collection == "collection03" {
                            query = query.filter(Filter::Field(FieldFilter {
                                path: FieldPath::field(["relationId"]).expect("valid field"),
    ...
    assert_eq!(parallel_count, PARALLEL_DOCUMENTS);
    assert_measurement(
...
fn measure_listener_fanout(
    ...
    assert_eq!(listen_documents, PARALLEL_DOCUMENTS);
    assert_measurement(
...
fn seed_dataset(store: &Store, database: &DatabaseName) {
    ...
    for index in 0..DATASET_DOCUMENTS {
        let (path, fields) = if index < PARALLEL_DOCUMENTS {
            let collection = QUERY_COLLECTIONS[index % QUERY_COLLECTIONS.len()];
        ...
        } else if index < PARALLEL_DOCUMENTS + RECORD_DOCUMENTS {
            let id = if index == PARALLEL_DOCUMENTS {
                "oracle".to_owned()
            } else {
                format!("record-{index:06}")
            };
            let owner = if index == PARALLEL_DOCUMENTS {
        ...
        } else if index < PARALLEL_DOCUMENTS + RECORD_DOCUMENTS + COLLECTION_GROUP_DOCUMENTS {
            (
    ...
    store
        .commit(&[Write::Set {
            key: DocumentKey::new(database.clone(), "records/oracle/parts/first")
...
fn verify_editor_listeners_and_leave_result_set(store: &Store, database: &DatabaseName) {
    let presentation =
        DocumentKey::new(database.clone(), "records/oracle").expect("valid presentation");
    ...
    let slides =
        Query::new(QueryScope::collection("records/oracle/parts").expect("valid slide scope"));
    ...
    let active_query = Query::new(QueryScope::collection("records").expect("valid scope")).filter(
