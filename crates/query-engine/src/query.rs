...
/// directly over the store's ordered scope iterator. Unlimited ordered disk
...
impl Iterator for QueryDocumentIterator {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match &mut self.inner {
            ...
            QueryDocumentIteratorInner::OrderedDisk(documents) => documents.next(),
            QueryDocumentIteratorInner::Buffered(documents) => documents.next(),
        }
    }
}

// Disk documents are decoded allocations, unlike the shared memory-store Arcs.
// Sorting only the order values avoids retaining every unrelated payload until
// the stream drains. Keep the original snapshot for every subsequent lookup.
struct OrderedDiskQueryDocumentIterator {
    snapshot: Snapshot,
    keys: std::vec::IntoIter<DocumentKey>,
    projection: Option<Vec<FieldPath>>,
}

impl Iterator for OrderedDiskQueryDocumentIterator {
    type Item = QueryDocument;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let document = self
            .snapshot
            .get(&key)
            .expect("ordered candidate must remain present in the same immutable snapshot");
...
fn ordered_disk_iterator(
    ...
    // None denotes the resource-name key; Some is an actual Firestore value,
    // including Null. All missing fields have already been excluded.
    ...
    let keys = candidates
        ...
        .collect::<Vec<_>>();
    QueryDocumentIterator {
        inner: QueryDocumentIteratorInner::OrderedDisk(Box::new(
            OrderedDiskQueryDocumentIterator {
