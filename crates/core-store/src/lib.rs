...
impl Store {
    ...
    /// Whether operations can perform blocking durable I/O. This inspects only
    /// the backend tag, without acquiring a snapshot or a store mutex.
    #[must_use]
...
impl Snapshot {
    ...
    /// Whether reading documents decodes them from the disk backend.
    #[must_use]
    pub const fn is_disk_backed(&self) -> bool {
        matches!(self.documents, SnapshotDocuments::Disk(_))
    }
