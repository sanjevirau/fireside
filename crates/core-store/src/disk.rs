...
struct PersistedMutation {
    key: DocumentKey,
    // Share the already immutable commit-plan value until WAL/redb persistence
    // finishes. Arc encodes identically to Document; no on-disk schema change.
...
impl Journal {
    fn open(path: &Path) -> Result<(Self, Vec<WalRecord>), DiskError> {
        ...
        #[cfg(windows)]
        {
            ...
            // Win32 write-through also requests NTFS metadata flushing. Keep
            // sync_all below: it flushes the newly created empty file as well.
...
mod tests {
    ...
    fn wal_record_shares_immutable_commit_documents_without_deep_clones() {
        ...
        let plan = store
            ...
            .memory
            .plan(&[Write::Create {
                key: key("items/shared"),
                fields: fields(Value::Array(vec![Value::Integer(1); 10_000])),
            }])
            .unwrap();
        let record = WalRecord::from_plan(&plan);
        assert!(Arc::ptr_eq(
            record.mutations[0].document.as_ref().unwrap(),
            plan.changes[0].after.as_ref().unwrap()
        ));
    ...
    #[test]
    fn wal_record_keeps_the_original_owned_document_wire_format() {
        // Frozen pre-optimization layout: field order and owned Document values.
        // Pointer ownership must never change persisted FSWAL001 bytes.
        #[derive(Encode, Decode)]
        struct OriginalRecord {
            revision: Revision,
            commit_time: Timestamp,
            mutations: Vec<(DocumentKey, Option<Document>)>,
        }
        let directory = TestDirectory::new();
        let store = DiskStore::open(directory.path(), DiskOptions::default()).unwrap();
        let nested = Value::Map(BTreeMap::from([
            ("text".into(), Value::String("火🔥".into())),
            ("bytes".into(), Value::Bytes(Arc::from([0, 1, 255]))),
            (
                "array".into(),
                Value::Array(vec![Value::Null, Value::Boolean(true)]),
            ),
            ("float".into(), Value::Double(f64::NAN)),
            ("zero".into(), Value::Double(-0.0)),
            ("vector".into(), Value::Vector(vec![1.0, f64::INFINITY])),
            ("number".into(), Value::Integer(i64::MIN)),
            (
                "point".into(),
                Value::GeoPoint {
                    latitude: 1.5,
            ...
            (
                "reference".into(),
                Value::Reference("projects/p/databases/(default)/documents/c/d".into()),
            ),
        ]));
        let plan = store
            .state()
        ...
        let original = OriginalRecord {
            ...
            mutations: plan
                .changes
                .iter()
                .map(|change| (change.key.clone(), change.after.as_deref().cloned()))
                .chain(std::iter::once((key("items/deleted"), None)))
                .collect(),
        };
        let mut current = WalRecord::from_plan(&plan);
        current.mutations.push(PersistedMutation {
            key: key("items/deleted"),
            document: None,
        });
        let expected = encode(&original).unwrap();
        assert_eq!(encode(&current).unwrap(), expected);
        let decoded: WalRecord = decode(&expected).unwrap();
        assert_eq!(encode(&decoded).unwrap(), expected);
        // Exercise the old payload with the production framed journal reader
        // and startup replay, not only a serialization round trip.
        drop(store);
        ...
        file.write_all(&u32::try_from(expected.len()).unwrap().to_le_bytes())
            .unwrap();
        file.write_all(&crc32c::crc32c(&expected).to_le_bytes())
            .unwrap();
        file.write_all(&expected).unwrap();
        file.sync_all().unwrap();
        drop(file);
        let reopened = DiskStore::open(directory.path(), DiskOptions::default()).unwrap();
        let document = reopened.snapshot().get(&key("items/unicode")).unwrap();
        assert_eq!(
            encode(document.as_ref()).unwrap(),
            encode(original.mutations[0].1.as_ref().unwrap()).unwrap()
        );
        assert!(reopened.snapshot().get(&key("items/deleted")).is_none());
